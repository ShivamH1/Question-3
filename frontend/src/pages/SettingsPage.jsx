...
// This is the SettingsPage component which displays user settings and allows the user to update their budget and categories
const SettingsPage = () => {

  // Access the user login details and user settings from the state context
  ...
  const deleteHandler = async (cat) => {
    // If there is only one category, show an error message and return
    ...
    try {
      // Create a new setting object with the deleted category removed
      const newSetting = {
        ...userSetting,
        categories: userSetting.categories.filter((c, i) => c !== cat),
      };
      // Send a request to the server to update the user settings
      ...
      // Update the user settings in the state context
          ...
          <div className="form-control">
            <label className="input-group">
              <Formik
                ...
                validate={(values) => {
                  const errors = {};
                  // If the budget is 0, show an error message
                  if (values.budget === 0) errors.budget = "Cannot be 0";
                  return errors;
                }}
                onSubmit={async (values, actions) => {
                  try {
                    // Create a new setting object with the updated budget
                    const newSetting = {
                      ...userSetting,
      ...
      <div className="overflow-x-auto">
        ...
        <div className="card-body items-center">
          <Formik
            ...
            onSubmit={async (values, actions) => {
              try {
                // Create a new setting object with the new category added
                const newSetting = {
                  ...userSetting,
